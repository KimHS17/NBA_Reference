    ...
    <div id="board_area">
        ...
        <div id="page_num">
          ...
          { //만약 page가 1보다 크거나 같다면
            echo "<li class='fo_re'>처음</li>"; //처음이라는 글자에 남색 표시 
          }else{
          ...
          for($i=$block_start; $i<=$block_end; $i++){ 
            ...
            if($page == $i){ //만약 page가 $i와 같다면 
              echo "<li class='fo_re'>[$i]</li>"; //현재 페이지에 해당하는 번호에 굵은 남색을 적용한다
            }else{
              echo "<li><a href='?page=$i'>[$i]</a></li>"; //아니라면 $i
            }
          }
          if($page >= $total_page){//만약 현재 페이지가 총 페이지 수 보다 크거나 같다면 빈 값
          }else{
            $next = $page + 1; //next변수에 page + 1을 해준다.
            echo "<li><a href='?page=$next'>다음</a></li>"; //다음글자에 next변수를 링크한다. 현재 4페이지에 있다면 +1하여 5페이지로 이동하게 된다.
          }
          if($page >= $total_page){ //만약 page가 페이지수보다 크거나 같다면
            echo "<li class='fo_re'>마지막</li>"; //마지막 글자에 긁은 남색을 적용한다.
          }else{
        ...
        <div id="write_btn">
            <button onclick="location.href='write.php'" style="cursor:pointer">글쓰기</button>
        </div>
    </div>
</body>
